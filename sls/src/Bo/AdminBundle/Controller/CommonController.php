...
use Bo\AdminBundle\Entity\Closegroup;
use Bo\AdminBundle\Entity\Absences;
use Bo\AdminBundle\Entity\Ccdate;
use Bo\AdminBundle\Entity\DateTransformer;
...
class CommonController extends Controller
{
	...
	protected function isHolidayForAll($ddate){
		$oEmHolodays = $this->getDoctrine()->getManager()->getRepository('BoAdminBundle:Holidayslist');
		$aHolidays = $oEmHolodays->findByDdate($ddate);
		if(isset($aHolidays[0]) and $aHolidays[0]->getProvince()=="All") return 1;
		else return 0;
	}
	...
	*/
	...
	protected function notifyContacts($ticket){
		...
		if($email!=null){
			...
			$res = $this->sendmail($email,$subject,$body,$cc,"jmnvekounou@gmail.com");
		}
		return $res;
	}
	...
	protected function getAmDaySchedule($amhour,$employee,$oSchedule,$sStatusAbsence,$oDate,$sStudentAbsence,$sSubstitution=null){
		...
		$aReturn = array('label'=>$sLibAm,'hour'=>$amhour,'option'=>1,'idcontract'=>$idcontract,'idgroup'=>$idgroup,'etsc'=>$etsc,'etsg'=>$etsg,'idagenda'=>$oSchedule->getId());
		return $this->addAbsencesTo($aReturn,$sStatusAbsence,$sStudentAbsence,$oDate,$oSchedule,$sSubstitution);
	}
	protected function getPmDaySchedule($pmhour,$employee,$oSchedule,$sStatusAbsence,$oDate,$sStudentAbsence,$sSubstitution=null){
		...
		$aReturn = array('label'=>$sLibPm,'hour'=>$pmhour,'option'=>1,'idcontract'=>$idcontract,'idgroup'=>$idgroup,'etsc'=>$etsc,'etsg'=>$etsg,'idagenda'=>$oSchedule->getId());
		//return $aReturn;
		return $this->addAbsencesTo($aReturn,$sStatusAbsence,$sStudentAbsence,$oDate,$oSchedule,$sSubstitution);
	}
	...
	private function getAmScheduledesc($oSchedule,$oDate){
		...
		if($this->isHolidaysBy($oDate,$oSchedule)==true) $sEventLib = $sEventLib."<br><br>".$this->getHoliday($oDate);
	...
	private function addAbsencesTo($aReturn,$sStatusAbsence,$sStudentAbsence,$oDate,$oSchedule,$sSubstitution){
		if($this->isHolidaysBy($oDate,$oSchedule)==true){
			$aReturn['absence'] = 1;
		}else{
	...
	protected function getScheduleByKey($aSchedule,$oDate,$option){
		$aResult = array();
		foreach($aSchedule as $oSchedule){
			if($option==1 and $oSchedule->getStartam()) $key = $this->getRealTime($oSchedule->getStartam()); 
			if(($option==2 or $option==3) and $oSchedule->getStartpm()) $key = $this->getRealTime($oSchedule->getStartpm());
			if($this->existDaySchedule($oSchedule,$oDate)==1 and isset($key)){
				if(isset($aResult[$key])) $key = $key*2;
	...
	private function getByDateAndTime($idemployee,$oDate,$option){
		...
		foreach($aSubstitution as $oSubstitution){
			if($this->getAmHour($oSubstitution)>0 and $option==1) return $oSubstitution;
			if($this->getPmHour($oSubstitution)>0 and $option==2) return $oSubstitution;	
		}
		return null;
	}
	protected function getByDateAndTimeBis($idemployee,$oDate,$option){
		...
		$aSubstitution = $this->getByHolderAndDate($idemployee,$oDate);
		foreach($aSubstitution as $oSubstitution){
			if($this->getAmHour($oSubstitution)>0 and $option==1) $aResult[] = $oSubstitution;
			if($this->getPmHour($oSubstitution)>0 and $option==2) $aResult[] = $oSubstitution;	
		}
		return $aResult;
	}
		...
		if(count($aSchedule)==0 and count($aSubstitution)>0 and $this->existSubs($aSubstitution,1,$employee->getId()) and $this->isWeekend($oDate)==false){
			...
			foreach($aSchedule as $oSchedule){
				...
				if(isset($aSubstitution[0]) and $this->existSubs($aSubstitution,1,$employee->getId()) and $this->isWeekend($oDate)==false){
					...
					//echo "Hour: ".$amhour."<br>";
					//echo "Exist Day: ".$this->existDaySchedule($oSchedule,$oDate)."<br>";
...
//exit(0);
	...
	public function isAbsentEmployee($employee,$oDate,$option){
		...
		$oAbsence=$aAbsence[0];
		if($option==3 and $oAbsence->getAmorPm()=="ALL") return true;
		if($oAbsence->getAmorPm()=="ALL" and ($option==1 or $option==2)) return true;
		if($oAbsence->getAmorPm()=="AM" and $option==1) return true;
		if($oAbsence->getAmorPm()=="PM" and $option==2) return true;
		return false;
	}
	...
	private function getAbsenceForSchedule($oSchedule,$oDate,$option){
		...
		if(count($aAbsence)>0) return $this->getEmployeeAbsenceStatus($employee,$aAbsence,$option);
		return $this->getScheduleCancel($oSchedule,$oDate,$option);
	}
	private function getEmployeeAbsenceStatus($employee,$aAbsence,$option){
		$status = null;
		if(isset($aAbsence[0])){
			if(($aAbsence[0]->getAmorpm()=="AM" or $aAbsence[0]->getAmorpm()=="ALL") and $option==1) $sLabel = $this->getLang()=="en"?" absent this morning":" absent ce matin";
			if(($aAbsence[0]->getAmorpm()=="PM" or $aAbsence[0]->getAmorpm()=="ALL") and $option==2) $sLabel = $this->getLang()=="en"?" absent this afternoon":" absent cet après-midi";
	...
	private function getScheduleCancel($oSchedule,$oDate,$option){
		...
		$status = null;
		if(isset($aCancel[0])){
			if(($aCancel[0]->getAmorpm()=="AM" or $aCancel[0]->getAmorpm()=="ALL") and $option==1) $sLabel = $this->getLang()=="en"?" Cancelled this morning":" Annule ce matin";
			if(($aCancel[0]->getAmorpm()=="PM" or $aCancel[0]->getAmorpm()=="ALL") and $option==2) $sLabel = $this->getLang()=="en"?" Cancelled this afternoon":" Annule cet après-midi";
			if(isset($sLabel)) $status = $sLabel.": ".$aCancel[0]->getMotif();
		}
		return $status;
	}
	...
	private function isEmployeeAbsent($employee,$oDate,$option){
		$aAbsence = $this->getEmployeeAbsenceByDate($employee,$oDate);
		if(isset($aAbsence[0])){
			if($aAbsence[0]->getAmorpm()=="ALL" or ($aAbsence[0]->getAmorpm()=="AM" and $option==1) or ($aAbsence[0]->getAmorpm()=="PM" and $option==2)) return 1;
		}
		return 0;
	}
	...
	private function getAbsenceForStudent($oContract,$oDate,$option){
		...
		if(isset($aAbsence[0]) and $oAbsence=$aAbsence[0]){
			...
			if(($oAbsence->getAmorpm()=="AM" or $oAbsence->getAmorpm()=="ALL") and $option==1){
			...
			if(($oAbsence->getAmorpm()=="PM" or $oAbsence->getAmorpm()=="ALL") and $option==2){
				$sLabel = $sLabel = $this->getLang()=="en"?$student." absent this afternoon":$student." absent cet après-midi";
				if($this->getNoShow($oAbsence,$oDate,$option)==1)  $sLabel = $sLabel." (No-show)";
				else $sLabel = $sLabel.$this->getTransCancel();
			}
		}		
	...
	private function getNoShow($oAbsence,$oDate,$option){
		...
		if($nj<$oAbsence->getNoshow()){
			if($oAbsence->getNsam()==true and $option==1) return 1;
			elseif($oAbsence->getNspm()==true and $option==2) return 1;
		}
		return 0;
	}
	...
	public function getStudentAbs($oContract,$oDate,$option){
		...
		$aAbsence = $this->getStudentAbsByDate($oStudent,$oDate);
		if(isset($aAbsence[0]) and $oAbsence=$aAbsence[0]){
			if($oAbsence->getAmorpm()=="ALL") return $oAbsence;
			elseif($oAbsence->getAmorpm()=="AM" and $option==1) return $oAbsence;
			elseif($oAbsence->getAmorpm()=="PM" and $option==2)  return $oAbsence;
		}
		return null;
	}
	...
	*/
	private function isStudentAbsent($oSchedule,$oDate,$option){
		...
		$aAbsence = $this->getStudentAbsByDate($oStudent,$oDate);
		if(isset($aAbsence[0]) and $oAbsence=$aAbsence[0]){
			if($oAbsence->getAmorpm()=="ALL") return true;
			if(($oAbsence->getAmorpm()=="AM" and $option==1) or ($oAbsence->getAmorpm()=="PM" and $option==2)) return true;
		}		
		return false;
	}
	...
	protected function getGroupAbsences($employee,$aTSC){
		...
		foreach($aTSC as $oTSC){
			if($oTSC->getGroup() and count($aAbsence)==1){
				if($aAbsence[0]->getAmorpm()=="ALL" or ($aAbsence[0]->getAmorpm()=="AM" and $this->getRealAmHour($oTSC)>0) or ($aAbsence[0]->getAmorpm()=="PM" and $this->getRealPmHour($oTSC)>0)) $aResult[$oTSC->getGroup()->getId()]=1;
			}			
		}
		return $aResult;
	}
	...
	private function existDaySubstitution($aSubstitution,$option,$idemployee){
		$aResult = array();
		foreach($aSubstitution as $oSubstitution){
			if($this->getAmHour($oSubstitution)>0 and $option==1 and $idemployee==$oSubstitution->getIdsubstitute()) 					$aResult[] = $oSubstitution;
			if($this->getPmHour($oSubstitution)>0 and $option==2 and $idemployee==$oSubstitution->getIdsubstitute()) 					$aResult[] = $oSubstitution;
		}
		return $aResult;
	}
	...
	protected function getObjectStudentBy($contract){
		...
		$oGroup = $contract->getGroup();
		if($oGroup and $oGroup->getContract()) return $oGroup->getContract();
		return null;
	}
	...
	private function getAmEventDesc($oEntity,$employee,$student,$room,$sAdvisor,$option,$oDate=null,$sStatusAbsence=null,$sStudentAbsence=null,$sSubstitution=null){
		$sEventLib =  $oEntity->getStartam()->format("H:i")." - ".$oEntity->getEndam()->format("H:i")."<br>".$employee->getFirstname()." ".$employee->getName().$this->getDayAction($oDate).$student.$this->getTransat().$room.$this->transCp().$sAdvisor;
		if($option==2) $sEventLib = $sEventLib."<br><br>".$employee->getFirstname()." ".$employee->getName().$this->transReplace().$this->getSubstitutionHolder($oEntity);
		if($option==1 and $this->isHolidaysBy($oDate,$oEntity)==true) $sEventLib = $sEventLib."<br><br>".$this->getHoliday($oDate);
		if($sStatusAbsence!=null) $sEventLib = $sEventLib."<br><br>".$sStatusAbsence;
	...
	private function getPmEventDesc($oEntity,$employee,$student,$room,$sAdvisor,$option,$oDate=null,$sStatusAbsence=null,$sStudentAbsence=null,$sSubstitution=null){
		$sEventLib = $oEntity->getStartpm()->format("H:i")." - ".$oEntity->getEndpm()->format("H:i")."<br>".$employee->getFirstname()." ".$employee->getName().$this->getDayAction($oDate).$student.$this->getTransat().$room.$this->transCp().$sAdvisor;
		if($option==2) $sEventLib = $sEventLib."<br><br>".$employee->getFirstname()." ".$employee->getName().$this->transReplace().$this->getSubstitutionHolder($oEntity);
		if($option==1 and $this->isHolidaysBy($oDate,$oEntity)==true) $sEventLib = $sEventLib."<br><br>".$this->getHoliday($oDate);
		if($sStatusAbsence!=null) $sEventLib = $sEventLib."<br><br>".$sStatusAbsence;
		if($sStudentAbsence!=null) $sEventLib = $sEventLib."<br><br>".$sStudentAbsence;
	...
	protected function getHoliday($oDate){
		$return = $this->getLang()=="en"?" Holidays: ":" Jour férié: ";
		$oHolidays = $this->getHolidaysBy($oDate);
		if($oHolidays){
	...
	protected function getCampusBySchedule($oSchedule){
		$oGroup = $oSchedule->getGroup();
		if($oGroup){
	...
	protected function getProvinceBySchedule($oSchedule){
		$oContract = $oSchedule->getContracts();
		if($oContract and $oContract->getAdresse()!=null){
			$sAddress = $oContract->getAdresse();
			if(strpos($sAddress,"Gatineau")!=false or strpos($sAddress,"gatineau")!=false){
				return "Québec";
			}
			if(strpos($sAddress,"Ottawa")!=false or strpos($sAddress,"ottawa")!=false){
				return "Ontario";
			}
		} 
		return null;	
	}
	protected function isHolidaysBy($oDate,$oSchedule){
		$oHoliday = $this->getHolidaysBy($oDate);
...
//en cours 28/12/2017
	...
	protected function checkAvailability($oSchedule,$oEmployee){
		...
		foreach($aOverlap as $id=>$aTab){
			...
			}elseif($oSchedule->getDescription()!=null){
				$message = $message.$oSchedule->getDescription()." another one with id ".$id;	
			}		
		}
		if(isset($aGoup)){
	...
	protected function getStudentBySubs($oEmployee){
    ...
    */
	...
	protected function checkSchedule($oEntity){
		...
		if($this->getTotalHourBy($oEntity)==$oEntity->getHourperday()) return 1;
		elseif($oEntity->getHourperday()==7){
			//reduce the pause time of total of schedules
			//The learner is authorize to two breacks
			//0.5 equal to two times of pause of 15 minutes
			$newtotalHour = $this->getTotalHourBy($oEntity) - 0.50; 
			if($newtotalHour==$oEntity->getHourperday()) return 1;
		}elseif($oEntity->getHourperday()>=3 and $oEntity->getHourperday()<7){
			//reduce the pause time of total of schedules
			//The learner needed to one break
			//0.25 equal to 15 minutes of break
			$newtotalHour = $this->getTotalHourBy($oEntity) - 0.25; 
			if($newtotalHour==$oEntity->getHourperday()) return 1;
		}elseif($oEntity->getHourperday()>0 and $oEntity->getHourperday()<3){
			//No break
			$newtotalHour = $this->getTotalHourBy($oEntity); 
			if($newtotalHour==$oEntity->getHourperday()) return 1;
		}
		return 0;
	}
	...
	*/
	...
	protected function getStudentEvents($oStudent,$aDates){
		$aAM=$this->getStudentDayEvent($oStudent,$aDates,1);
		$aPM=$this->getStudentDayEvent($oStudent,$aDates,2);
		//exit(0);
		return array('am'=>$aAM,'pm'=>$aPM);
	}
	protected function getStudentDayEvent($oStudent,$aDates,$option){
		$aResult = array();
		foreach($aDates as $key=>$oDate){
			...
			foreach($aSchedule as $index=>$oSchedule){
				$aEvent = $this->getDaySchStudEvent($oSchedule,$oDate,$option,$aSubst);
	...
	public function getStudentDaySchedule($oStudent,$oDate,$option){
		...
		$aSchedule = $oRep->getScheduleByContract($oStudent,$oDate);
		//Return the result
		return $this->getScheduleByDay($aSchedule,$oDate,$option);
	}
	...
	protected function getEmployeeScheduled($employee,$oDate,$option){
		...
		foreach($aSubstitution as $oSub){
			echo "Subs:<br>";
			echo $oSub->getStartpm()->format("H:i")."<br>";
			echo $oSub->getEndpm()->format("H:i")."<br><br>";
			echo "Sch:<br>";
			foreach($aSchedule as $index=>$oSch){
			echo $oSch->getStartpm()->format("H:i")."<br>";
			echo $oSch->getEndpm()->format("H:i")."<br>";
				if($oSch->getStartpm()<$oSub->getStartpm() and $oSub->getStartpm()<$oSch->getEndpm() and !isset($aSubs[$oSub->getId()])){
echo $oSch->getStartpm()->format("H:i")." < ".$oSub->getStartpm()->format("H:i")." and ".$oSub->getStartpm()->format("H:i")." < ".$oSch->getEndpm()->format("H:i")."<br>";
					//$aResult[] = array('type='=>2,'object'=>$oSub);
$aResult[] = array('type='=>2);
					$aSubs[$oSub->getId()] = $oSub->getId();
				}elseif($oSch->getEndpm()<=$oSub->getStartpm()){
echo $oSch->getEndpm()->format("H:i")." <= ".$oSub->getStartpm()->format("H:i")."<br>";
...
echo "i+1: ".$aSchedule[$index+1]->getStartpm()->format("H:i")." > ".$oSub->getEndpm()->format("H:i")."<br>";
					//$aResult[] = array('type='=>2,'object'=>$oSub);
					$aSubs[$oSub->getId()] = $oSub->getId();
$aResult[] = array('type='=>2);
				}else{
...
$aResult[] = array('type='=>1);
	...
	public function getStudentHour($oStudent,$oDate,$option){
		$hour = 0;
		//Check absence for the student, if true return 0
	...
	public function getStudentTeacher($oStudent,$oDate,$option){
		$hour = 0;
		//Check absence for the student, if true return 0
		$bStudAbs = $this->isStudentAbsent2($oStudent,$oDate,$option);
		if($bStudAbs==true) return $hour;
	...
	public function getGroupHour($oGroup,$oDate,$option){
		$hour = 0;
		//Check absence for the group, if true return 0
		//$bGroupAbs = $this->isStudentAbsent2($oGroup,$oDate,$option);
	...
	*/ 
	...
	private function getDayHourScheduled($oSchedule,$oDate,$option,$aSubst){
		...
		if($oSchedule->getContracts()){
			...
			if($oSchedule->getGroup()==null or ($oContract!=null and count($oContract->getStudents())==1))
	...
	private function getHourSubByOption($oSubstitution,$option){
		if($this->getAmHour($oSubstitution)>0 and $option==1){ 
			$amhour = $this->getAmHour($oSubstitution);
		...
		if($this->getPmHour($oSubstitution)>0 and $option==2 and $oSubstitution->getStartpm()<$this->getTime(15,30)){
			$pmhour = $this->getPmHour($oSubstitution);
			...
			return $pmhour;	
		}elseif($this->getPmHour($oSubstitution)>0 and $option==3){
			$pmhour = $this->getPmHour($oSubstitution);
			if($pmhour<=4 and $oSubstitution->getHour()<=$pmhour) return $oSubstitution->getHour();
			if($pmhour>=3) return $this->getPmHourWithBreak($oSubstitution);
			return $pmhour;	
	...
	private function getHourSchByOption($oSchedule,$option){
		if($this->getAmHour($oSchedule)>0 and $option==1){ 
			$amhour = $this->getAmHour($oSchedule);
		...
		if($this->getPmHour($oSchedule)>0 and $option==2 and $startpm<$this->getRealTime("15:30")){
			$pmhour = $this->getPmHour($oSchedule);
			if($oSchedule->getContracts()!=null or $oSchedule->getGroup()!=null){
			...
			return $pmhour;	
		}elseif($this->getPmHour($oSchedule)>0 and $option==3 and $startpm>$this->getRealTime("15:30")){
			$pmhour = $this->getPmHour($oSchedule);
			if($oSchedule->getContracts()!=null or $oSchedule->getGroup()!=null){
				if($pmhour<=4 and $oSchedule->getHourperday()<=$pmhour) return $oSchedule->getHourperday();
				if($pmhour>=3) return $this->getPmHourWithBreak($oSchedule);
	...
	protected function getDayScheduleEvent($oSchedule,$oDate,$option,$aSubs=null){
		...
		$hour = $option==1?$this->getAmHour($oSchedule):$this->getPmHour($oSchedule);	
		if($oSubstitution!=null and $this->getAmHour($oSubstitution)>0 and $option==1 and !isset($aSubs[$oSubstitution->getId()])){
			return $this->getSubstByOption($oSubstitution,$oSchedule,$oDate,$option);
		}elseif($oSubstitution and $this->getPmHour($oSubstitution)>0 and $option==2 and !isset($aSubs[$oSubstitution->getId()])){
			return $this->getSubstByOption($oSubstitution,$oSchedule,$oDate,$option);
		}
	...
	private function getDaySubsEvent($oSubstitution,$oSchedule,$oDate,$option,$aSubs){
		...
		if($oSubstitution!=null and $this->getAmHour($oSubstitution)>0 and $option==1 and !isset($aSubs[$oSubstitution->getId()])){
			return $this->getSubstByOption($oSubstitution,$oSchedule,$oDate,$option);
		}elseif($oSubstitution!=null and $this->getPmHour($oSubstitution)>0 and $option==2 and !isset($aSubs[$oSubstitution->getId()])){
			return $this->getSubstByOption($oSubstitution,$oSchedule,$oDate,$option);
		}
	}
	...
	private function timeisBetween($oSubstitution,$oSchedule,$option){
		if($option==1 and $oSubstitution->getStartam()>=$oSchedule->getStartam() and $oSubstitution->getStartam()<$oSchedule->getEndam()) return true;
		if($option==1 and $oSubstitution->getEndam()>=$oSchedule->getStartam() and $oSubstitution->getEndam()<$oSchedule->getEndam()) return true;
		if($option==2 and $oSubstitution->getStartpm()>=$oSchedule->getStartpm() and $oSubstitution->getStartpm()<$oSchedule->getEndpm()) return true;
		if($option==2 and $oSubstitution->getEndpm()>=$oSchedule->getStartpm() and $oSubstitution->getEndpm()<$oSchedule->getEndpm()) return true;
		return false;
	}
	...
	private function getSubsBySchedule($oSchedule,$oDate,$option){
		...
		foreach($aSubstitution as $oSubstitution){
			if($this->getAmHour($oSubstitution)>0 and $option==1){
				return $oSubstitution;
			}elseif($this->getPmHour($oSubstitution)>0 and $option==2){
				 return $oSubstitution;	
			}
		}
		return null;
	...
	public function getSubsForSchedule($oSchedule,$oDate,$option){
		...
		foreach($aSubstitution as $oSubstitution){
			...
			foreach($aAgenda as $oAgenda){
				//Check if the hour 
				if($this->getAmHour($oAgenda)>0 and $this->getAmHour($oSubstitution)>0 and $option==1 and $this->existDaySchedule($oAgenda,$oDate)==1){
				...
				if($this->getPmHour($oAgenda)>0 and $this->getPmHour($oSubstitution)>0 and $option==2 and $this->existDaySchedule($oAgenda,$oDate)==1){
	...
	public function getSubsForStudent($oSchedule,$oDate,$option){
		...
		foreach($aSubstitution as $oSubstitution){
			if($this->getAmHour($oSubstitution)>0 and $option==1 and $this->existDaySchedule($oSubstitution,$oDate)==1) 	$aResult[]=$oSubstitution;
			if($this->getPmHour($oSubstitution)>0 and $option==2 and $this->existDaySchedule($oSubstitution,$oDate)==1) 	$aResult[]=$oSubstitution;
		}
		return $aResult;
	}
	...
	private function getSubstByOption($oSubstitution,$oSchedule,$oDate,$option,$idsubstitute=null){
		...
		if($this->getAmHour($oSubstitution)>0 and $option==1) return $this->getAmDaySubstitution($hour,$oEmployee,$oSubstitution,$oDate);
		if($this->getPmHour($oSubstitution)>0 and $option==2) return $this->getPmDaySubstitution($hour,$oEmployee,$oSubstitution,$oDate);
		return null;
	}
	...
	private function getScheduleByOption($hour,$oSchedule,$oDate,$option){
		...
		if($option==1)
			return $this->getAmDaySchedule($hour,$oSchedule->getEmployee(),$oSchedule,$sStatusAbsence,$oDate,$sStudentAbsence,$sSubstitution);
		else	
			return $this->getPmDaySchedule($hour,$oSchedule->getEmployee(),$oSchedule,$sStatusAbsence,$oDate,$sStudentAbsence,$sSubstitution);
	}
	...
	private function getDaySubstitution($aSubstitution,$oSchedule,$employee,$oDate,$option){
		...
		foreach($aSubstitution as $oSubstitution){
			if($oSubstitution->getStartdate()>=$oSchedule->getStartdate() and $oSubstitution->getEnddate()<=$oSchedule->getEnddate()){
				$hour = $oSubstitution->getHour();
				$daySubEvent = $option==1?$this->getAmDaySubstitution($hour,$oEmployee,$oSubstitution,$oDate):$this->getPmDaySubstitution($hour,$oEmployee,$oSubstitution,$oDate);
			 	return $daySubEvent;
			}
		}
		return null;
	...
	public function checkAmAndPm($substitution){
		if($oAbsence->getAmorpm()=="ALL") return 1;
		$ham = floatval($this->getRealAmHour($substitution)); 
		$hpm = floatval($this->getRealPmHour($substitution));
		if($ham>0 and $oAbsence->getAmorpm()=="AM") return 1; 
		if($hpm>0 and $oAbsence->getAmorpm()=="PM") return 1; 
		return 0;
	}
	...
	protected function compareThem($oldContract,$contract){
		...
		if($oldContract->getClicsignaturedate()!=$contract->getClicsignaturedate()){
			...
			$aResult[]=array('field'=>"Clic signature date",'old'=>$old,"new"=>$new);
		}
		if($oldContract->getClicsignaturedate()!=$contract->getClicsignaturedate()){
			$old = $oldContract->getClicsignaturedate()?$oldContract->getClicsignaturedate()->format("Y-m-d"):null;
			$new = $contract->getClicsignaturedate()?$contract->getClicsignaturedate()->format("Y-m-d"):null;
	...
	*/
	protected function notifyAdminMembers($agenda){
		...
		$body = $this->renderView("BoAdminBundle:Agenda:notification.html.twig", array('agenda'=>$agenda,'user'=>$this->getConnectedUser()));
	...
	*/
	protected function notifyForSubstitution($oSubstitution,$agenda){
		...
		if($employee==$oHolder and $oSubstitute instanceof Employee){
			...
			$body = $this->renderView("BoAdminBundle:Substitution:notification.html.twig", array('agenda'=>$agenda,'substitution'=>$oSubstitution,'holder'=>$oHolder,'substitute'=>$oSubstitute,'user'=>$this->getConnectedUser()));
			$res = $this->sendmail($advisor_email,$subject,$body,$cc,"jnvekounou@yahoo.fr");
			return $res;
		}
		return null;			
	...
	protected function checkDay($ddate,$entity){
		return $this->existDayInEntity($ddate->format("l"),$entity);
	}
	...
	private function getHourBySchedule($schedule,$option,$param){
		$hour = 0;
		if($option==3 and $this->getAmHour($schedule)>0 and $this->getAmHour($schedule)>0){
	...
	private function getStudentAbsBool($oSchedule,$oDate,$param,$option=null){
		...
		if(isset($aAbsence[0]) and $oAbsence=$aAbsence[0]){
			...
			if($oAbsence->getAmorpm()=="AM" and $option==1){
				return true;
			}
			if($oAbsence->getAmorpm()=="PM" and $option==2){
				return true;
			}
		}		
		return false;
	...
	protected function getTicketFormView(){
        	...
        	$form = $this->createForm('Bo\AdminBundle\Form\HelpType', $this->getTicketForm());
		return $form->createView();
	}
	/*
	*@param integer $end
